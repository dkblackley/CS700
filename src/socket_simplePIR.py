...
class PIRServer:
    ...
    def initialize_database(self, params: PIRParams, N: int):
        ...
        np.random.seed(0)
        self.db = np.random.randint(0, params.p, (sqrt_N, sqrt_N), dtype=np.int64)
...
class PIRClient:
    ...
    def query_index(self, i: int, db_dims: Tuple[int, int]) -> int:
        ...
        start_time = time.time()
        self.send_data(self.socket, "setup")
        setup_data = self.receive_data(self.socket)
        ...
        self.send_data(self.socket, "answer")
        query_size = self.send_data(self.socket, query)
        answer = self.receive_data(self.socket)
        
        result = self.pir.recover(state, self.client_hint, answer, self.pir.params)
